                    ...
                    state_save_fct state_save, uint32_t save_intvl)
{
    ...
    bsec_library_return_t bsec_status = BSEC_OK;
    bsec_library_return_t debug_status = BSEC_OK;
    ...
    while (1)
    {
        ...
        debug_status =  bsec_sensor_control(time_stamp, &sensor_settings);
        if(debug_status != BSEC_OK){
            printf("bsec_sensor_control returns error-code : %d \n",debug_status);
        }
