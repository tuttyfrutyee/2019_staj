...
#define BATTERY_PROFILE_APP_IDX                   1

#define EXAMPLE_DEVICE_NAME                       "Tuttyfruty"
...
static uint16_t battery_level_handle_table[BTRY_IDX_NB];

static uint8_t test_manufacturer[3]={'E', 'S', 'P'};

static uint8_t sec_service_uuid[32] = {
    /* LSB <--------------------------------------------------------------------------------> MSB */
...
static void gatts_heart_rate_profile_event_handler(esp_gatts_cb_event_t event,
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param);

static void gatts_battery_profile_event_handler(esp_gatts_cb_event_t event,
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)       ;                     

/* One gatt-based profile one app_id and one gatts_if, this array will store the gatts_if returned by ESP_GATTS_REG_EVT */
static struct gatts_profile_inst profile_tab[PROFILE_NUM] = {
    [HEART_PROFILE_APP_IDX] = {
        .gatts_cb = gatts_heart_rate_profile_event_handler,
...
static const uint16_t character_presentations_uuid = ESP_GATT_UUID_CHAR_PRESENT_FORMAT;
...
//Battery Service - Battery Level Characteristic, notify
static const uint8_t battery_level_value = 48;
static const uint16_t battery_level_uuid = ESP_GATT_UUID_BATTERY_LEVEL;
static const uint8_t battery_level_descr_prsnt[7] = {0x04, 0x00, 0xAD, 0x27, 0x01, 0x00, 0x00}; 
static const uint8_t battery_level_ccc[2] = {0x01,0x00};
...
static const esp_gatts_attr_db_t heart_rate_gatt_db[HRS_IDX_NB] =
{
    ...
    [HRS_IDX_HR_MEAS_VAL]             =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&heart_rate_meas_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
...
//Full Battery Service Database Description 
static const esp_gatts_attr_db_t battery_gatt_db[BTRY_IDX_NB] = {
    ...
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&battery_level_uuid, ESP_GATT_PERM_READ,
      HRPS_HT_MEAS_MAX_LEN, sizeof(battery_level_value), (uint8_t*)&battery_level_value}},

     // Battery Level Characteristic  - Characteristic Presentation Format
    [BTRY_IDX_BLVL_PRSNT]        =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_presentations_uuid, ESP_GATT_PERM_READ,
      sizeof(uint16_t)*4, sizeof(battery_level_descr_prsnt), (uint8_t *)&battery_level_descr_prsnt}},
     

    // Battery Level Characteristic - Client Characteristic Configuration Descriptor
    [BTRY_IDX_BLVL_NTF_CFG]        =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_client_config_uuid, ESP_GATT_PERM_READ|ESP_GATT_PERM_WRITE,
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            ...
            esp_ble_gatts_create_attr_tab(battery_gatt_db, gatts_if,
                                      BTRY_IDX_NB, BATTERY_SVC_INST_ID);
            break;
        case ESP_GATTS_READ_EVT:
            break;
        case ESP_GATTS_WRITE_EVT:
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ESP_LOGI(GATTS_TABLE_TAG, "The number handle = %x",param->add_attr_tab.num_handle);
            if (param->create.status == ESP_GATT_OK){
                if(param->add_attr_tab.num_handle == BTRY_IDX_NB) {
                    memcpy(battery_level_handle_table, param->add_attr_tab.handles,
                    sizeof(battery_level_handle_table));
                    esp_ble_gatts_start_service(battery_level_handle_table[BTRY_IDX_SVC]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to HRS_IDX_NB(%d)",
                         param->add_attr_tab.num_handle, HRS_IDX_NB);
                }
...
void app_main(void)
{
    ...
    //register heart profile
    ret = esp_ble_gatts_app_register(HEART_PROFILE_APP_IDX);
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gatts app register error, error code = %x", ret);
        return;
    }
    //register battery profile
    ret = esp_ble_gatts_app_register(BATTERY_PROFILE_APP_IDX);
    if (ret){
        ESP_LOGE(GATTS_TABLE_TAG, "gatts app register error, error code = %x", ret);
        return;
    }
