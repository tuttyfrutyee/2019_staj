...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param){
    ...
    ESP_LOGI(GATTS_TAG, "inside example_write_event_env");
    if (param->write.need_rsp){
        ESP_LOGI(GATTS_TAG, "inside write.need_rsp");
        if (param->write.is_prep){
            ESP_LOGI(GATTS_TAG, "inside write.is_prep");
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        if (!param->write.is_prep){
            ...
            ESP_LOGI(GATTS_TAG,"boolean : %d",gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle );
            if (gl_profile_tab[PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2){
                uint16_t descr_value = param->write.value[1]<<8 | param->write.value[0];
                ESP_LOGI(GATTS_TAG,"write.value[1] = %d",param->write.value[1]);
                ESP_LOGI(GATTS_TAG,"write.value[0] = %d",param->write.value[0]);
...
static void prepareButtonAndLed(){
    ...
    gpio_isr_handler_add(19, gpio_isr_handler, 0 );
}
